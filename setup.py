...
class BaseBuildCommand(Command):
    ...
    def initialize_options(self):
        self.build_lib = None
        self.force = None
        self.work_path = os.path.join(ROOT_PATH, "src/meteor/static/meteor")
        self.inplace = None
...
class BuildAssetsCommand(BaseBuildCommand):
    ...
    def _get_package_version(self):
        ...
        try:
            ...
            log.info(f"pulled version information from 'meteor' module. {meteor.__file__}")
            version = self.distribution.get_version()
            build = meteor.__build__
        finally:
            sys.path.pop(0)
    ...
    def _build(self):
        ...
        if not version_info["version"] or not version_info["build"]:
            log.fatal("Could not determine meteor version or build")
            sys.exit(1)
    ...
    @property
    def meteor_static_dist_path(self):
        return os.path.abspath(os.path.join(self.build_lib, "src/static/meteor/dist"))
